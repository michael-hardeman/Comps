...
package Protection is
   ...
   --  Closed_Semaphore, *except* for the task that owns currently the
   --  semaphore. The function Holder returns the Task_ID of the task that
   --  currently holds the semaphore, or Null_Task_ID if it is free (this can
   --  also be used to query whether the semaphore is currently being held).
   --  If there are more V's than P's, or if a V is performed by a task not
   --  holding the semaphore, Semaphore_Error is raised and the semaphore
   ...
   --  In the case where several parameters are needed, it is generally
   --  possible to pack them into a record, and use this generic.
   ...
   package Generic_Protected_Call is
      type Parametered_Procedure_Access
         is access procedure (Item : Parameter_Type);
