...
package body Tracer.Assert  is
   ...
   procedure Check (Assumption : Boolean; Message : Wide_String) is
   begin
   ...
   begin
      if Detector_Table (Item.Num).Caller = Current_Task then
         Detector_Table (Item.Num).Level :=
            Detector_Table (Item.Num).Level - 1;
   ...
   begin
      if Detector_Table (Item.Num).Caller = Null_Task_Id then
         ...
         Trace ("Reentrancy detected with " &
            To_Wide_String (Image (Detector_Table (Item.Num).Caller)) &
            ": " & Wide_String (Item.Message.all) &
            " / " & Wide_String (Detector_Table (Item.Num).Message.all));
      end if;
   end Initialize;
