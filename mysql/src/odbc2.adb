...
package body Odbc2 is
   ...
   pragma Unreferenced (Null_Dbc);
   Null_Stmt : constant HSTMT := HSTMT (System.Null_Address);
   pragma Unreferenced (Null_Stmt);
   ...
   begin
      if Result = SQL_SUCCESS or else Result = SQL_NO_DATA_FOUND then
         ...
         declare
            ...
            Rc       : RETCODE;
            pragma Unreferenced (Rc);
            Message  : constant String := Error_Text (Environment,
                                                      HDBC (Dbc),
                                                      HSTMT (Stmt));
            Sqlstate : String (1 .. 5);
         begin
   ...
   --------------------------------
   procedure Disconnect (Connection : Connection_Handle) is
   begin
      --
   ...
   procedure Commit (Connection : Connection_Handle) is
   begin
      Check_Retcode (SQLTRANSACT (Null_Env, HDBC (Connection), SQL_COMMIT),
                     "Commit", Connection);
   end Commit;

   procedure Rollback (Connection : Connection_Handle) is
   begin
      Check_Retcode (SQLTRANSACT (Null_Env, HDBC (Connection), SQL_ROLLBACK),
                     "Rollback", Connection);
   end Rollback;

   procedure Auto_Commit (Connection : Connection_Handle;
                          On         : Boolean)
   is
   ...
   ------------------------------
   function Is_Open (Cursor : Cursor_Type) return Boolean is
   begin
   ...
   function Describe_Column (Cursor : Cursor_Type;
                             Column : Natural) return Column_Descriptor is
      Result : Column_Descriptor;
      Rc : RETCODE;

      --  This variable is written to in SQLDESCRIBECOL
   ...
   begin
      ...
      Rc := SQLFETCHSCROLL (HSTMT (Cursor.Statement),
                            Dir,
                            Interfaces.Integer_32 (Offset));
   ...
   begin
      return Cursor.Fields (Column).Value_Length;
   end Get_Length;
   ...
   procedure Bind_Output (Cursor        : in out Cursor_Type;
                          ...
                          Variable_Addr :        System.Address;
                          Variable_Size :        Integer := 0)
   is
   begin
   ...
   begin
      ...
      Rc := SQLBINDPARAMETER (HSTMT (Cursor.Statement),         --  hstmt
                              ...
                              To_A_SDWORD (Cursor.Params        --  pcbValue
