...
package Binary_Map is
   ...
   function  Fetch (From : Map; Key : Key_Type) return Value_Type;
   --  Get a value from Map
   --  Raises Not_Present if Key not found

   function  Fetch (From : Map; Key : Key_Type; Default_Value : Value_Type)
                      return Value_Type;
   --  Get a value from Map
   ...
   function  Is_Present (Within : Map; Key : Key_Type) return Boolean;
   ...
   generic
      with procedure Action (Key : Key_Type; Value : in out Value_Type);
